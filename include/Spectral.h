...
distr_t Get_Laplace_transfo_piecewise(double mean, double sigma, double Estart, double E1, double E2, double ratio, int T, int tmax, int prec, string SMEARING_FUNC, const function<PrecFloat(const PrecFloat&, const PrecFloat&,const PrecFloat&,const PrecFloat&, int)> &f, const distr_t_list& corr, double &syst,const double mult, double& pull, string MODE, string reg_type, string CORR_NAME, double Ag_ov_A0_target, bool JackOnKer, const distr_t &Prefact,const  double &offset,  string analysis_name, Vfloat &covariance, const function<double(const function<double(double)>&)> &syst_func, bool Use_guess_density, const function<double(double)> &guess_density,  bool Int_up_to_Max=false, double Max_Erg=4.0, double b=2.99, bool ONLY_FW=0,  bool GENERALIZED_NORM=0, const function<PrecFloat(const PrecFloat &, const PrecFloat &, const PrecFloat &, const PrecFloat &, int)> F_NORM = [](const PrecFloat E, const PrecFloat m, const PrecFloat s, const PrecFloat E0, int jack_id) { return 1.0;}  , const function<PrecFloat( PrecFloat )> Atr_gen_NORM= [](PrecFloat t) {return 0.0;} );
