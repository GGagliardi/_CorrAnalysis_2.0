...
void Get_optimal_lambda(const PrecMatr &Atr,const PrecMatr &Atr_std_norm, const PrecMatr &Atr_std_norm_Emax, const PrecMatr &B,const PrecVect &ft,vector<PrecVect>& ft_jack,  const PrecVect &ft_std_norm, const PrecVect &ft_std_norm_Emax, const PrecFloat & M2, const PrecFloat &M2_std , const PrecFloat &M2_std_Emax, const double &mean, const double &sigma, const double &Estart,  double& lambda_opt, vector<PrecVect> Rt_n, const PrecVect &M_n ,vector<PrecVect>& M_n_jack, const distr_t_list & corr,int T, int tmin, int tmax, const double mult,   string MODE, string curr_type, string SMEARING_FUNC, string CORR_NAME, double Ag_ov_A0_tg, bool JackOnKer, const distr_t &Prefact, const double &offset,  string analysis_name, const function<PrecFloat(const PrecFloat&, const PrecFloat&,const PrecFloat&,const PrecFloat&, int)> &f , const function<double(const function<double(double)>&)> &syst_func, bool Use_guess_density, const function<double(double)> &guess_density);
