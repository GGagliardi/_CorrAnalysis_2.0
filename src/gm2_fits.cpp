	  ...
	  for( auto& n_m_pair: Inc_log_corr_list) {
	    ...
	    if(Fit_allowed) {
	      ...
	      bf.ansatz= [=](const W_fpar& X, const W_ipar& Y) {
			   ...
			   return X.w0*(1.0 + X.Am*(csi-csi_phys) + X.Plog*csi*log(csi/csi_phys)+ par_art*art + par_art_a4*pow(Y.a,4))*(1.0  + (X.Al1 +par_vol_art*pow(Y.a,2))*csi*(1.0/pow(MpL,1.5))*exp(-MpL)); // pion mass Y.Mp is in [GeV]
			 };
		...
		cout<<"#######################################################"<<endl;
	      ...
	      Ch2.push_back( Bt_w_fit.get_ch2_ave());
	      Aka_weight.push_back( exp( -0.5*(Bt_w_fit.get_ch2_ave() + 2.0*npars - Nmeas)));
	      Aka_weight_red.push_back( exp(-0.5*(Bt_w_fit.get_ch2_ave() + 2.0*npars -Nmeas + small_sample_corr)));
		...
		if(t_sin_fit != "OS") data_extr_TM.distr_list.push_back((meas_tm.distr_list[iens]/(1.0 + (Al1 + Al2_tm*a_iens*a_iens)*csi*distr_t::f_of_distr(FVE, MpiL))-1.0e-10*w0*Am*(csi-csi_phys)));
		if(t_sin_fit != "tm") data_extr_OS.distr_list.push_back((meas_OS.distr_list[iens]/(1.0 + (Al1 + Al2_OS*a_iens*a_iens)*csi*distr_t::f_of_distr(FVE, MpiL))-1.0e-10*w0*Am*(csi-csi_phys)));
	      }
	      ...
	      string Fit_tag= "a2_"+t_a2+"_FSEs_"+t_FSEs+"_a4_"+t_a4+"_Mp_"+t_mass+"_sfit_"+t_sin_fit+"_n_"+to_string(n_m_pair.first)+"_m_"+to_string(n_m_pair.second);

	      Print_To_File({}, {csi,mL,alat, tm_OS, func_W_val, func_W_err}, "../data/gm2/"+channel+"/windows_fit_func/"+W_type+"/"+Fit_tag+".dat","", "#  xi   Mpi*L a(GeV^-1) tm/OS  val  err");

	      //Print data corrected for FSEs and physical point extrapolation
	      boost::filesystem::create_directory("../data/gm2/"+channel+"/tm/windows_extr_data");
	      boost::filesystem::create_directory("../data/gm2/"+channel+"/OS/windows_extr_data");
	      boost::filesystem::create_directory("../data/gm2/"+channel+"/tm/windows_extr_data/"+W_type);
	      boost::filesystem::create_directory("../data/gm2/"+channel+"/OS/windows_extr_data/"+W_type);
  
  
	      if(t_sin_fit != "OS") {
		Print_To_File(Ens_Tag, {L_list, a_distr_list.ave(), data_extr_TM.ave(), data_extr_TM.err()    }, "../data/gm2/"+channel+"/tm/windows_extr_data/"+W_type+"/"+Fit_tag+".dat", "", "#Ens L a val");
	      }
	      if(t_sin_fit != "tm") {
		Print_To_File(Ens_Tag, {L_list, a_distr_list.ave(), data_extr_OS.ave(), data_extr_OS.err()    }, "../data/gm2/"+channel+"/OS/windows_extr_data/"+W_type+"/"+Fit_tag+".dat", "", "#Ens L a val");
	      }
